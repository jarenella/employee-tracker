        ...
        db.query('SELECT * FROM employees WHERE role_id = 3', (error, res) => {
            ...
            .then(ans => {
                ...
                db.query(`SELECT id FROM employees WHERE first_name = "${managerFirstName}" AND last_name = "${managerLastName}"`, (err, res) => {
                    //result of id is turned into a string containing only numbers (no alphanumeric), then turned to just an integer and saved as a managerID variable
                    const managerID = parseInt(JSON.stringify(res).replace(/\D/g,''));

                    //query the db to find the ID of the role the user selected by NAME
                    db.query(`SELECT id FROM roles WHERE title = "${ans.role}"`, (err, res) => {
                        ...
                        //query the db to add an employee based off of the users specifications
                        db.query(`INSERT INTO employees (first_name, last_name, role_id, manager_id) VALUES ("${ans.firstName}", "${ans.lastName}", ${roleID}, ${managerID})`, (err, res) => {
                            //query the db to show an updated list of all employees
                            db.query("SELECT * FROM employees", (err, res) => {
                                console.log("Here is an updated list of all employees");
                                console.log(res);
                            })
                        })
                    })
